...
PROJECT_HOME = "/cs/usr/bareluz/gabi_labs/nematus_clean"
DEBIAS_FILES_HOME = PROJECT_HOME + "/debias_files/"
MT_GENDER_HOME = PROJECT_HOME + "/mt_gender/"
OUTPUTS_HOME = PROJECT_HOME + "/debias_outputs/"
DATA_HOME = "/cs/snapless/gabis/bareluz/debias_nmt_data"
ENGLISH_VOCAB = DATA_HOME + '/en_vocab_merged.txt'
TRANSFORMERS_CACHE = "/cs/snapless/gabis/bareluz"

from enum import Enum
from datetime import datetime

lang_to_gender_specific_words_map = {'ru': ['он', 'она'], 'de': ['er', 'sie'], 'he': ['הוא', 'היא'],
                                     'en': ['he', 'she'], 'es': ['él', 'ella']}
...
def get_debias_files_from_config(config_str):
    ...
    now = datetime.now()
    SANITY_CHECK_FILE = OUTPUTS_HOME + "en-" + lang + "/debias/sanity_check_" + now.strftime(
        "%d-%m-%Y_%H-%M-%S") + ".csv"
